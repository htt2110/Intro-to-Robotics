...
class Estimator(object):
    ...
    def estimate(self, sens):
        ...
        h_list = self.H_matrix(X_next_hat, sens,num,valid)
        print 'h_list', h_list
        
        #-----------W matrix-------------#

        W = self.covariance_sensor(sens,num,valid)
        print 'W = ', W

        #-----------S list---------------#
        s_list = numpy.zeros((2*valid,2*valid))
        if valid !=0:
        ...
        landmark_estimate = self.predicted_data(X_next_hat,sens,num,valid)
        print 'landmark_estimate', landmark_estimate

        new = numpy.zeros((2*valid,1))
        new = self.innovation(sens,new,landmark_estimate,valid)
        print 'new', new       
...
#        print 'X_next_hat', X_next_hat 
    ...
    def F_matrix(self, sens):
        ...
        fmatrix[2][2] = 1
    	return fmatrix
    def predicted_data(self,X_next_hat,sens,num,valid):
        new_list = numpy.zeros((2*valid,1))
        if valid!= 0:
            J = 0
    ...
    def H_matrix(self, X_next_hat, sens,num,valid):
    	
        
        J = 0

    	if valid!= 0:
            h_list = numpy.zeros((2*valid,3))
            for i in range(len(sens.readings)):
                if i in L:
                    continue
    ...
    def innovation(self,sens,new,landmark_estimate,valid):
        if valid!=0:
            for i in range(len(sens.readings)):
                if i in L:
                    continue
    ...
    def covariance_sensor(self,sens,num,valid):
